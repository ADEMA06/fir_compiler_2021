...
void fir::xml_writer::do_function_definition_node(fir::function_definition_node *const node, int lvl){
    ...
    openTag("default_val", lvl + 2);
    if (node->defaultValue()) {
      node->defaultValue()->accept(this, lvl + 4);
    }
    closeTag("default_val", lvl + 2);
    ...
    openTag("function_body", lvl + 2);
    if(node->function_body()) node->function_body()->accept(this, lvl + 4);
    closeTag("function_body", lvl + 2);

    closeTag(node, lvl);
}

void fir::xml_writer::do_function_body_node(fir::function_body_node *const node, int lvl) {
    openTag(node, lvl);
    openTag("prologue", lvl + 2);
    if(node->prologue()) node->prologue()->accept(this, lvl + 4);
    ...
    if(node->epilogue()) node->epilogue()->accept(this, lvl + 4);
